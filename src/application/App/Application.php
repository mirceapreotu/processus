...
class App_Application extends Lib_Application
{
     ...
     * @return bool
     */
    public function isFacebookApplication()
    {
        $socialEnv = $this->getSocialEnvProvider();
        $haystack = array("fb");
        return (bool)(in_array($socialEnv, $haystack)===true);
    }

    /**
     * @return bool
     */
    public function isVzApplication()
    {
        $socialEnv = $this->getSocialEnvProvider();
        $haystack = array("vz");
        return (bool)(in_array($socialEnv, $haystack)===true);
...
;
        /*
        if ($protocolFromRequest==="https") {
            $useHttps = true;
        }
        if ((trim(strtolower($hostFromRequest))) === (trim(strtolower($hostHttps)))) {
            $useHttps = true;
        }
    ...
    public function isUrlSsl($url)
    {
        ...
        if ($scheme === Lib_Url_Uri::SCHEME_HTTPS) {
            return true;
        }

        $host = $zendUri->getHost();

        $hostSSL = $this->getUrlHostSsl();

        if ((is_string($hostSSL)) && ($host===$hostSSL)) {
            return true;
        }
