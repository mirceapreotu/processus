...
class ClientLogin
{
    ...
    $loginUri = self::CLIENTLOGIN_URI, $accountType = 'HOSTED_OR_GOOGLE')
    {
        ...
        $client->setConfig(
        array('maxredirects' => 0, 'strictredirects' => true, 
        'useragent' => $useragent));
        
        $client->setEncType('multipart/form-data');
        
        $postParams = array('accountType' => $accountType, 
        'Email' => (string) $email, 'Passwd' => (string) $password, 
        'service' => (string) $service, 'source' => (string) $source);
        ...
        if ($response->getStatusCode() == 200) {
            ...
            $client->setConfig(
            array('strictredirects' => true, 'useragent' => $useragent));
            return $client;
