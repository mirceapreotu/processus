...
class GApps extends GData
{
    ...
    $applicationId = 'MyCompany-MyApp-1.0')
    {
        ...
        array('service' => self::AUTH_SERVICE_NAME));
        $this->_domain = $domain;
    }
    ...
    public function getEmailListRecipientEntry ($location)
    {
        ...
        'Zend\GData\GApps\EmailListRecipientEntry');
    }
    ...
    public function insertNickname ($nickname, $uri = null)
    {
        ...
        'Zend\GData\GApps\NicknameEntry');
        return $newEntry;
    }
    ...
    {
        if ($uri === null) {
            throw new App\InvalidArgumentException('URI must not be null');
        }
        $newEntry = $this->insertEntry($member, $uri, 
        'Zend\GData\GApps\MemberEntry');
        return $newEntry;
    }
    ...
    public function insertEmailList ($emailList, $uri = null)
    {
        ...
        'Zend\GData\GApps\EmailListEntry');
        return $newEntry;
    }
    ...
    public function __call ($method, $args)
    {
        if (preg_match('/^new(\w+Query)/', $method, $matches)) {
            ...
            if ($foundClassName != null) {
                ...
                $args = array_merge(array($this->getDomain()), 
                $args);
    ...
    {
        $this->delete(
        $this->getBaseUrl() . self::APPS_EMAIL_LIST_PATH . '/' . $emailList);
    }
    ...
    public function removeRecipientFromEmailList ($recipientAddress, $emailList)
    {
        $this->delete(
        $this->getBaseUrl() . self::APPS_EMAIL_LIST_PATH . '/' . $emailList .
         self::APPS_EMAIL_LIST_RECIPIENT_POSTFIX . '/' . $recipientAddress);
    }
