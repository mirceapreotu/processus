...
class YouTube extends Media
{
    ...
    $applicationId = 'MyCompany-MyApp-1.0', $clientId = null, $developerKey = null)
    {
        ...
        $this->setHttpClient($client, $applicationId, $clientId, 
        $developerKey);
    }
    ...
    public static function parseFormUploadTokenResponse ($response)
    {
        ...
        if ($tokenText != null && $urlText != null) {
            ...
            throw new App\Exception('Form upload token not found in response');
        }
    }
    ...
    $url = 'http://gdata.youtube.com/action/GetUploadToken')
    {
        if ($url != null && is_string($url)) {
            ...
            throw new App\Exception('Url must be provided as a string URL');
        }
    }
    ...
    public function getActivityForUser ($username)
    {
        if ($this->getMajorProtocolVersion() == 1) {
            ...
            'User activity feeds ' . 'are not available in API version 1.');
        }
    ...
    public function replyToCommentEntry ($commentEntry, $commentText)
    {
        ...
        '\Zend\GData\YouTube\CommentEntry');
    }
