...
use Zend\Json\Exception\RecursionException, Zend\Json\Exception\InvalidArgumentException;
...
class Encoder
{
    ...
    protected function _encodeString (&$string)
    {
        ...
        // 0x0c => \f
        $string = str_replace(
        array(chr(0x08), chr(0x0C)), array('\b', '\f'), $string);
    ...
    public static function encodeClass ($className, $package = '')
    {
        ...
        if (! $cls->isInstantiable()) {
            ...
            "'{$className}' must be instantiable");
        }
        ...
        for ($i = 0; $i < $strlen_var; $i ++) {
            ...
            switch (true) {
                ...
                case (($ord_var_c & 0xF0) == 0xE0):
                    ...
                    // see http://www.cl.cam.ac.uk/~mgk25/unicode.html#utf-8
                    $char = pack('C*', 
                    $ord_var_c, ord($value[$i + 1]), ord($value[$i + 2]));
                    $i += 2;
                    $utf16 = self::_utf82utf16($char);
                    $ascii .= sprintf('\u%04s', bin2hex($utf16));
                    break;
