...
class Encoder
{
     ...
     * @var array
     */
    protected $_options = array('sort' => true, 'version' => 1, 'wrap' => 78);
    ...
    protected function _encode ($value)
    {
        if (is_scalar($value)) {
            ...
            if (is_array($value)) {
                ...
                if ($value instanceof \Zend\Ldap\Node) {
        ...
        foreach ($value as $v) {
            ...
            if (isset($this->_options['wrap']) &&
             strlen($attribute) > $this->_options['wrap']) {
                $attribute = trim(
                chunk_split($attribute, $this->_options['wrap'], PHP_EOL . ' '));
            }
    ...
    protected function _encodeAttributes (array $attributes)
    {
        ...
        if (isset($this->_options['sort']) && $this->_options['sort'] === true) {
            ...
            if (array_key_exists('objectclass', $attributes)) {
                ...
                $attributes = array_merge(array('objectclass' => $oc), 
                $attributes);
            }
