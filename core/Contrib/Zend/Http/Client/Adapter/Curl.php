...
use Zend\Http\Client\Adapter as HttpAdapter, Zend\Http\Client\Adapter\Exception as AdapterException, Zend\Http\Client, Zend\Http\Request;
...
class Curl implements HttpAdapter, Stream
{
    ...
    public function __construct ()
    {
        ...
        $this->invalidOverwritableCurlOptions = array(CURLOPT_HTTPGET, 
        CURLOPT_POST, CURLOPT_UPLOAD, CURLOPT_CUSTOMREQUEST, CURLOPT_HEADER, 
        CURLOPT_RETURNTRANSFER, CURLOPT_HTTPHEADER, CURLOPT_POSTFIELDS, 
        CURLOPT_INFILE, CURLOPT_INFILESIZE, CURLOPT_PORT, CURLOPT_MAXREDIRS, 
        CURLOPT_CONNECTTIMEOUT, CURL_HTTP_VERSION_1_1, CURL_HTTP_VERSION_1_0);
    }
    ...
    $body = '')
    {
        ...
        if ($this->outputStream) {
            ...
            curl_setopt($this->curl, CURLOPT_FILE, 
            $this->outputStream);
        } else {
        ...
        if ($method == 'POST') {
            ...
            CURLOPT_INFILE, $this->config['curloptions'][CURLOPT_INFILE]);
            ...
            foreach ((array) $this->config['curloptions'] as $k => $v) {
                if (! in_array($k, $this->invalidOverwritableCurlOptions)) {
                    if (curl_setopt($this->curl, $k, $v) == false) {
                        throw new AdapterException\RuntimeException(
                        sprintf("Unknown or erroreous cURL option '%s' set", $k));
                    }
                }
            }
        }
