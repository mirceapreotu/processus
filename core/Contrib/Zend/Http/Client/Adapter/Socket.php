...
class Socket implements HttpAdapter, Stream
{
     ...
     * @var array
     */
    protected $config = array('persistent' => false, 'ssltransport' => 'ssl', 
    'sslcert' => null, 'sslpassphrase' => null, 'sslusecontext' => false);
    ...
    public function connect ($host, $port = 80, $secure = false)
    {
        ...
        $host = ($secure ? $this->config['ssltransport'] : 'tcp') . '://' .
         $host;
        ...
        if (! is_resource($this->socket) || ! $this->config['keepalive']) {
            ...
            if (! $this->socket) {
                $this->close();
                throw new AdapterException\RuntimeException(
                'Unable to Connect to ' . $host . ':' . $port . '. Error #' .
                 $errno . ': ' . $errstr);
            }
    ...
    public function read ()
    {
                ...
                do {
                    ...
                    do {
                        ...
                        if ($this->out_stream) {
                            ...
                            $this->out_stream, $read_to - $current_pos) == 0) {
                                $this->_checkSocketReadTimeout();
                                break;
                            }
                        } else {
                            $line = @fread($this->socket, 
                            $read_to - $current_pos);
     ...
     // Else, if we got the content-length header, read this number of bytes
            ...
            for ($read_to = $current_pos + $contentLength; $read_to >
             $current_pos; $current_pos = ftell($this->socket)) {
