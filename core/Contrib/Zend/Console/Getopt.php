...
class Getopt
{
    ...
    self::CONFIG_PARSEALL => true, self::CONFIG_CUMULATIVE_PARAMETERS => false, 
    ...
    public function toJson ()
    {
        ...
        foreach ($this->_options as $flag => $value) {
            $j['options'][] = array(
            'option' => array('flag' => $flag, 'parameter' => $value));
        }
    ...
    protected function _parseSingleOption ($flag, &$argv)
    {
        ...
        if (! isset($this->_ruleMap[$flag])) {
            ...
            if (! $this->_getoptConfig[self::CONFIG_FREEFORM_FLAGS]) {
                ...
                "Option \"$flag\" is not recognized.", $this->getUsageMessage());
            }
    ...
    protected function _setNumericOptionValue ($value)
    {
        ...
        $this->_getoptConfig['numericFlagsOption'], $value);
    }
    ...
    {
        if (true === $value &&
         $this->_getoptConfig[self::CONFIG_CUMULATIVE_FLAGS]) {
    ...
    {
        $this->_options[$flag] = array_key_exists($flag, $this->_options) ? (int) $this->_options[$flag] +
         1 : true;
    }
