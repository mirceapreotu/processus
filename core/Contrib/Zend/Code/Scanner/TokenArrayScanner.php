...
use Zend\Code\Scanner, Zend\Code\NameInformation, Zend\Code\Exception, Zend\Code\Annotation\AnnotationManager;
    ...
    public function getClass ($classNameOrInfoIndex)
    {
        ...
        if (is_int($classNameOrInfoIndex)) {
            ...
            foreach ($this->infos as $infoIndex => $info) {
                if ($info['type'] === 'class' &&
                 $info['name'] === $classNameOrInfoIndex) {
                    $classFound = true;
                    break;
                }
            }
        ...
        ($info['tokenEnd'] - $info['tokenStart'] + 1)),  // zero indexed array
    ...
    protected function scan ()
    {
        ...
        &$docCommentIndex)
        {
            ...
            if ($validTrailingTokens === null) {
                $validTrailingTokens = array(T_WHITESPACE, T_FINAL, T_ABSTRACT, 
                T_INTERFACE, T_CLASS, T_FUNCTION);
            }
        ...
        switch ($tokenType) {
            ...
            case T_NAMESPACE:
                ...
                $infos[$infoIndex] = array('type' => 'namespace', 
                'tokenStart' => $MACRO_TOKEN_LOGICAL_START_INDEX(), 
                'tokenEnd' => null, 'lineStart' => $token[2], 
                'lineEnd' => null, 'namespace' => null);
            ...
            case T_USE:
                ...
                $infos[$infoIndex] = array('type' => 'use', 
                'tokenStart' => $MACRO_TOKEN_LOGICAL_START_INDEX(), 
                'tokenEnd' => null, 'lineStart' => $tokens[$tokenIndex][2], 
                'lineEnd' => null, 'namespace' => $namespace, 
            ...
            case T_REQUIRE_ONCE:
                ...
                $infos[$infoIndex] = array('type' => 'include', 
                'tokenStart' => $MACRO_TOKEN_LOGICAL_START_INDEX(), 
                'tokenEnd' => null, 'lineStart' => $tokens[$tokenIndex][2], 
                'lineEnd' => null, 
            ...
            case T_INTERFACE:
                ...
                if ($infos[$infoIndex]['shortName'] == '' &&
                 (($tokenType === T_CLASS || $tokenType === T_INTERFACE) &&
                 $infos[$infoIndex]['type'] === 'class' ||
                 ($tokenType === T_FUNCTION &&
                 $infos[$infoIndex]['type'] === 'function'))) {
                    $infos[$infoIndex]['shortName'] = $tokens[$tokenIndex + 2][1];
                    $infos[$infoIndex]['name'] = (($namespace != '-GLOBAL-') ? $namespace .
