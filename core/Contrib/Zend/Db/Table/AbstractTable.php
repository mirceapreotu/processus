...
abstract class AbstractTable
{
    ...
    public function getReference ($tableClassname, $ruleKey = null)
    {
        ...
        foreach ($refMap as $reference) {
            if ($reference[self::REF_TABLE_CLASS] ==
             '\\' . ltrim($tableClassname, '\\')) {
                return $reference;
            }
        }
        throw new Exception(
    ...
    public function setDefaultSource ($defaultSource = self::DEFAULT_NONE)
    {
        ...
        array(self::DEFAULT_CLASS, self::DEFAULT_DB, self::DEFAULT_NONE))) {
    ...
    protected function _setupMetadata ()
    {
        ...
        if (null !== $this->_metadataCache) {
            ...
            (isset($dbConfig['options']['port']) ? ':' .
             $dbConfig['options']['port'] : null) .
             (isset($dbConfig['options']['host']) ? ':' .
             $dbConfig['options']['host'] : null) . '/' . $dbConfig['dbname'] .
    ...
    protected function _setupPrimaryKey ()
    {
        ...
        if (! array_intersect((array) $this->_primary, $cols) ==
         (array) $this->_primary) {
            throw new Exception(
            "Primary key column(s) (" . implode(',', (array) $this->_primary) .
             ") are not columns in this table (" . implode(',', $cols) . ")");
        }
        ...
        if ($this->_sequence === true &&
         $this->_db instanceof \Zend\Db\Adapter\PDO\PGSQL) {
    ...
    public function info ($key = null)
    {
        ...
        $info = array(self::SCHEMA => $this->_schema, 
        self::NAME => $this->_name, self::COLS => $this->_getCols(), 
    ...
    public function select ($withFromPart = self::SELECT_WITHOUT_FROM_PART)
    {
        ...
        if ($withFromPart == self::SELECT_WITH_FROM_PART) {
            $select->from($this->info(self::NAME), Select::SQL_WILDCARD, 
                ...
                switch ($map[self::ON_UPDATE]) {
                    case self::CASCADE:
                        ...
                        for ($i = 0; $i < count($map[self::COLUMNS]); ++ $i) {
                            ...
                            $refCol = $this->_db->foldCase(
                            $map[self::REF_COLUMNS][$i]);
    ...
    public function find ()
    {
        ...
        if (count($whereList)) {
            ...
            foreach ($whereList as $keyValueSets) {
                ...
                foreach ($keyValueSets as $keyPosition => $keyValue) {
                    ...
                    $columnName = $this->_db->quoteIdentifier(
                    $keyNames[$keyPosition], true);
        ...
        if ($whereClause == null) {
            ...
            array('table' => $this, 'rowClass' => $this->getRowClass(), 
            'stored' => true));
        }
    ...
    public function fetchRow ($where = null, $order = null)
    {
        ...
        $data = array('table' => $this, 'data' => $rows[0], 
        'readOnly' => $select->isReadOnly(), 'stored' => true);
    ...
    public function createRow (array $data = array(), $defaultSource = null)
    {
        ...
        $config = array('table' => $this, 'data' => $defaults, 
        'readOnly' => false, 'stored' => false);
