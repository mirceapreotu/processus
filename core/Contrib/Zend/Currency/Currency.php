...
class Currency
{
     ...
     * @see Zend_Locale
     */
    protected $_options = array('position' => self::STANDARD, 
    'script' => null, 'format' => null, 'display' => self::NO_SYMBOL, 
    'precision' => 2, 'name' => null, 'currency' => null, 'symbol' => null, 
    'locale' => null, 'value' => 0, 'service' => null, 
    'tag' => 'Zend_Locale');
    ...
    public function __construct ($options = null, $locale = null)
    {
        if (is_array($options)) {
            ...
            if (Locale\Locale::isLocale($options, false)) {
    ...
    public function toCurrency ($value = null, array $options = array())
    {
        ...
        $original = $value;
        $value = Locale\Format::toNumber($value, 
        array('locale' => $locale, 'number_format' => $format, 
        ...
        if (empty($options['script']) === false) {
            $value = Locale\Format::convertNumerals($value, 'Latn', 
            $options['script']);
        }
    ...
    private function _extractPattern ($pattern, $value)
    {
        ...
        foreach ($patterns as $content) {
            if (strpos($content, '<') !== false) {
                ...
                $check = iconv_substr($content, 0, 
                iconv_strpos($content, '≤'));
                $token = iconv_substr($content, 
                iconv_strpos($content, '≤') + 1);
                if ($check <= $value) {
                    return $token;
                }
            }
    ...
    private function _checkParams ($currency = null, $locale = null)
    {
        ...
        'name' => $abbreviation, 'country' => $country);
    }
    ...
    public function getSymbol ($currency = null, $locale = null)
    {
        ...
        // Get the symbol
        $symbol = Cldr::getContent($params['locale'], 'currencysymbol', 
        $params['currency']);
    ...
    public function getShortName ($currency = null, $locale = null)
    {
        ...
        $list = Cldr::getContent($params['locale'], 'currencytoname', 
        $params['currency']);
        if (empty($list) === true) {
            $list = Cldr::getContent($params['locale'], 'nametocurrency', 
            $params['currency']);
            if (empty($list) === false) {
    ...
    public function getName ($currency = null, $locale = null)
    {
        ...
        // Get the name
        $name = Cldr::getContent($params['locale'], 'nametocurrency', 
        $params['currency']);
        if (empty($name) === true) {
            $name = Cldr::getContent($params['locale'], 'nametocurrency', 
            $params['name']);
    ...
    public function getRegionList ($currency = null)
    {
        ...
        $data = Cldr::getContent($this->_options['locale'], 'regiontocurrency', 
        $currency);
    ...
    public function setService ($service)
    {
        if (is_string($service)) {
            if (! class_exists($service)) {
                throw new Exception\InvalidArgumentException(
                'A currency service by class name "' . $service .
                 '" does not exist');
            }
            $service = new $service();
        }
        ...
        if (! ($service instanceof CurrencyService)) {
            throw new Exception\InvalidArgumentException(
            'A currency service must implement Zend\Currency\CurrencyService');
        }
            ...
            switch ($name) {
                ...
                case 'format':
                    if ((empty($value) === false) and
                     (Locale\Locale::isLocale($value) === false)) {
                        if (! is_string($value) ||
