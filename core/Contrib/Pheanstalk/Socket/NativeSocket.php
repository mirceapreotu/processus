...
class Pheanstalk_Socket_NativeSocket implements Pheanstalk_Socket
{
    ...
    {
        if (! $this->_socket = @fsockopen($host, $port, $errno, $errstr, 
        $connectTimeout)) {
    ...
    {
        do {
            $data = isset($length) ? fgets($this->_socket, $length) : fgets(
            $this->_socket);
            
            if (feof($this->_socket)) {
                throw new Pheanstalk_Exception_ConnectionException(666, 
