...
class NativeSocket implements \Pheanstalk\Socket
{
    ...
    public function __construct($host, $port, $connectTimeout)
    {
        ...
        if (!$this->_socket) {
            throw new \Pheanstalk\Exception\ConnectionException($errno, $errstr . " (connecting to $host:$port)");
        }
    ...
    public function write($data)
    {
        ...
        for ($written = 0, $fwrite = 0; $written < strlen($data); $written += $fwrite)
        {
            ...
            if ($history->isFullWithNoWrites()) {
                throw new \Pheanstalk\Exception\SocketException(sprintf(
    ...
    public function read($length)
    {
        ...
        while ($read < $length && !$this->_wrapper()->feof($this->_socket))
        {
            ...
            if ($data === false) {
                throw new \Pheanstalk\Exception\SocketException('fread() returned false');
            }
    ...
    public function getLine($length = null)
    {
        do
        {
            ...
            if ($this->_wrapper()->feof($this->_socket)) {
                throw new \Pheanstalk\Exception\SocketException("Socket closed by server!");
            }
        }
        while ($data === false);
