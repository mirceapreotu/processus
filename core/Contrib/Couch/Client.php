 ...
 * @package    Couch
 */
class Couch_Client extends Couch_Db
{
     ...
     *
     * key is the Couch_Client method (mapped with __call)
    ...
    "conflicts" => array("name" => "conflicts", "filter" => "staticValue", 
    "staticValue" => "true"), 
    "revs" => array("name" => "revs", "filter" => "staticValue", 
    "staticValue" => "true"), 
    "revs_info" => array("name" => "revs_info", "filter" => "staticValue", 
    "staticValue" => "true"), "rev" => array("name" => "rev", "filter" => null), 
    ...
    "stale" => array("name" => "stale", "filter" => "staticValue", 
    "staticValue" => "ok"), 
    "descending" => array("name" => "descending", 
    ...
    "include_docs" => array("name" => "include_docs", 
    "filter" => "jsonEncodeBoolean"), 
    "inclusive_end" => array("name" => "inclusive_end", 
    "filter" => "jsonEncodeBoolean"));
     ...
     */
    public static $allowed_underscored_properties = array('_id', '_rev', 
     ...
     */
    public static $underscored_properties_to_remove_on_storage = array(
    '_conflicts', '_revisions', '_revs_info');
     ...
     */
    protected function _queryAndTest ($method, $url, $allowed_status_codes, 
    $parameters = array(), $data = NULL)
    {
        // 		print_r($method.' '.$url."\n");
        // 		print_r($parameters);
    ...
    function __call ($name, $args)
    {
        ...
        if ($this->query_defs[$name]['filter'] == 'int') {
            $this->query_parameters[$this->query_defs[$name]['name']] = (int) reset(
            $args);
            ...
            $this->query_parameters[$this->query_defs[$name]['name']] = json_encode(
            reset($args));
        } elseif ($this->query_defs[$name]['filter'] == 'ensureArray') {
            if (is_array(reset($args))) {
                $this->query_parameters[$this->query_defs[$name]['name']] = reset(
                $args);
            }
        } elseif ($this->query_defs[$name]['filter'] == 'string') {
            $this->query_parameters[$this->query_defs[$name]['name']] = (string) reset(
            $args);
    ...
    public function useDatabase ($dbname)
    {
        ...
        if (! $this->isValidDatabaseName($dbname)) {
            ...
            'Database name contains invalid characters. Only lowercase characters (a-z), digits (0-9), and any of the characters _, $, (, ), +, -, and / are allowed.');
        }
        $this->dbname = $dbname;
        return $this;
    }
    ...
    {
        return $this->_queryAndTest('PUT', '/' . urlencode($this->dbname), 
        array(201));
    }
    ...
    {
        return $this->_queryAndTest('DELETE', '/' . urlencode($this->dbname), 
        array(200));
    }
    ...
    {
        return $this->_queryAndTest('GET', '/' . urlencode($this->dbname), 
        array(200));
    }
    ...
    {
        return $this->_queryAndTest("POST", 
        '/' . urlencode($this->dbname) . '/_compact', array(202));
    }
    ...
    {
        return $this->_queryAndTest("POST", 
        '/' . urlencode($this->dbname) . '/_view_cleanup', array(202));
    }
    ...
    public function filter ($value, $additional_query_options = array())
    {
        if (strlen(trim($value))) {
            ...
            $this->query_parameters = array_merge($additional_query_options, 
    ...
    {
        if (! empty($this->query_parameters['feed']) &&
         $this->query_parameters['feed'] == 'continuous') {
    ...
    public function getDoc ($id)
    {
        ...
        if (preg_match('/^_design/', $id)) {
            $url = '/' . urlencode($this->dbname) . '/_design/' .
             urlencode(str_replace('_design/', '', $id));
        } else {
    ...
    public function storeDoc ($doc)
    {
        ...
        foreach (array_keys(get_object_vars($doc)) as $key) {
            if (in_array($key, 
            Couch_Client::$underscored_properties_to_remove_on_storage)) {
                unset($doc->$key);
            } elseif (substr($key, 0, 1) == '_' and
             ! in_array($key, Couch_Client::$allowed_underscored_properties))
                throw new InvalidArgumentException(
                "Property $key can't begin with an underscore");
        }
        $method = 'POST';
    ...
    public function storeDocs ($docs, $all_or_nothing = false)
    {
        if (! is_array($docs)) {
            throw new InvalidArgumentException(
            "docs parameter should be an array");
        }
        /*
		...
		*/
        ...
        foreach ($docs as $doc) {
            if ($doc instanceof Couch_Document) {
    ...
    public function updateDoc ($ddoc_id, $handler_name, $params, $doc_id = null)
    {
        if (! is_array($params) && ! is_object($params)) {
            throw new InvalidArgumentException(
            "params parameter should be an array or an object");
        }
        if (is_object($params)) {
            $params = (array) $params;
        }
        $method = 'PUT';
        $url = '/' . urlencode($this->dbname) . '/_design/' . urlencode(
        $ddoc_id) . '/_update/' . $handler_name . '/';
        if ($doc_id) {
     ...
     */
    public function storeAsAttachment ($doc, $data, $filename, 
     ...
     * @return object CouchDB attachment storage response
     */
    public function storeAttachment ($doc, $file, 
    $content_type = 'application/octet-stream', $filename = null)
    {
        if (! is_object($doc)) {
            throw new InvalidArgumentException("Document should be an object");
        }
    ...
    public function deleteDoc ($doc)
    {
        ...
        $url = '/' . urlencode($this->dbname) . '/' . urlencode($doc->_id) .
         '?rev=' . urlencode($doc->_rev);
        return $this->_queryAndTest('DELETE', $url, array(200));
    }

    /**
     * returns couchDB results as Couch_Documents objects
     *
    ...
    public function getView ($id, $name)
    {
        if (! $id or ! $name) {
            throw new InvalidArgumentException(
            "You should specify view id and name");
        }
        $url = '/' . urlencode($this->dbname) . '/_design/' . urlencode($id) .
         '/_view/' . urlencode($name);
        if ($this->results_as_cd) {
        ...
        if (! $results_as_cd) {
            return $this->_queryAndTest($method, $url, array(200), $view_query, 
            $data);
        }
    ...
    $additionnal_parameters = array())
    {
        if (! $id or ! $name) {
            throw new InvalidArgumentException(
            "You should specify list id and name");
        }
        if (! $view_name) {
            throw new InvalidArgumentException("You should specify view name");
        }
        $url = '/' . urlencode($this->dbname) . '/_design/' . urlencode($id) .
         '/_list/' . urlencode($name) . '/' . urlencode($view_name);
        $this->results_as_cd = false;
        list ($method, $view_query, $data) = $this->_prepare_view_query();
     ...
     * @return object CouchDB list query response
     */
    public function getForeignList ($id, $name, $view_id, $view_name, 
    $additionnal_parameters = array())
    {
        if (! $id or ! $name) {
            throw new InvalidArgumentException(
        ...
        if (! $view_id or ! $view_name) {
            throw new InvalidArgumentException(
            "You should specify view id and view name");
        }
        $url = '/' . urlencode($this->dbname) . '/_design/' . urlencode($id) .
         '/_list/' . urlencode($name) . '/' . urlencode($view_id) . '/' .
         urlencode($view_name);
        $this->results_as_cd = false;
        list ($method, $view_query, $data) = $this->_prepare_view_query();
     ...
     * @return object CouchDB list query response
     */
    public function getShow ($id, $name, $doc_id = null, 
    $additionnal_parameters = array())
    {
        if (! $id or ! $name) {
            throw new InvalidArgumentException(
    ...
    public function compactViews ($id)
    {
        ...
        // 		print_r("calling $id");
        return $this->_queryAndTest("POST", 
        '/' . urlencode($this->dbname) . '/_compact/' . urlencode($id), 
        array(202));
    }
    ...
    {
        $response = $this->startkey('_design/')
            ->endkey('_designa')
            ->getAllDocs();
        // 		print_r($response);
    ...
    public function getUuids ($count = 1)
    {
        ...
        if ($count < 1) {
            throw new InvalidArgumentException(
            "Uuid count should be greater than 0");
        }
