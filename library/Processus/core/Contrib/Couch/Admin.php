...
class Couch_Admin
{
    ...
    public function createAdmin ($login, $password, $roles = array())
    {
        ...
        try {
            $raw = $this->client->query("PUT", $url, array(), json_encode($data));
        } catch (Exception $e) {
            throw $e;
        }
        ...
        $dsn["user"] = $login;
        $dsn["pass"] = $password;
        $client = new Couch_Client($this->build_url($dsn), $this->userdb, 
        $this->client->options());
        $user = new stdClass();
        $user->name = $login;
        $user->type = "user";
    ...
    public function createUser ($login, $password, $roles = array())
    {
        ...
        $user->roles = $roles;
        $user->_id = "org.couchdb.user:" . $login;
        $client = new Couch_Client($this->client->dsn(), $this->userdb, 
        $this->client->options());
        return $client->storeDoc($user);
    }
    ...
    public function getAllUsers ($include_docs = false)
    {
        ...
        return $client->startkey("org.couchdb.user:")
    ...
    public function addRoleToUser ($user, $role)
    {
        ...
        } elseif (! property_exists($user, "_id") ||
         ! property_exists($user, "roles")) {
            throw new InvalidArgumentException(
            "user parameter should be the login or a user document");
        }
    ...
    public function getSecurity ()
    {
        ...
        $raw = $this->client->query("GET", "/" . $dbname . "/_security");
        $resp = Couch_Db::parseRawResponse($raw);
        if ($resp['status_code'] != 200) {
            throw new Couch_Exception($raw);
        }
    ...
    public function setSecurity ($security)
    {
        ...
        $dbname = $this->client->getDatabaseName();
        $raw = $this->client->query("PUT", "/" . $dbname . "/_security", 
        array(), json_encode($security));
        $resp = Couch_Db::parseRawResponse($raw);
        if ($resp['status_code'] == 200) {
            return $resp['body'];
        }
