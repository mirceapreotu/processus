 ...
 * @version		$Id:$
 */
class App_JsonRpc_V1_Fb_Context extends Lib_JsonRpc_Context
{
    ...
    protected $_ignoreAuthClassMethodFilterConfig = array(
        ...
        "whitelist" => array(
            ...
            "*_Service_Facebook_Graph::*",

            "*_Service_Facebook_Pages_Events::*",
            "*_Service_Facebook_Pages_Admins::*",

            "*_Service_Facebook_Viewer_Permissions::*",
            "*_Service_Facebook_Viewer_Events::*",
     ...
     * @return array|null
     */
    public function decodeDeepLinkFromRequestIfExists()
    {
        $result = null;

        //$pageType = $this->getContext()->getPageTypeFromRequest();
        $pageUrl = $this->getPageUrlFromRequest();


        $managerDeeplink = $this->getApplication()
                ->getManagerDeeplink();

        $pageUri = $managerDeeplink->newUri(null);
        ...
        $deeplinkDecoded = $managerDeeplink->decodeDeepLink($pageUri);
        $deeplinkAction = Lib_Utils_Array::getProperty(
            $deeplinkDecoded, "action"
        );

        if ($managerDeeplink->isActionAvailable($deeplinkAction)!==true) {
            return $result;
        }
        ...
        switch($deeplinkAction) {
            ...
            default: {
                ...
                if (is_string($deeplinkAction)) {
                    return $deeplinkDecoded;
                }
    ...
    public function prepare()
    {
        ...
        $rpcClientId = Lib_Utils_Array::getProperty($rpc, "clientId");
        $this->getApplication()->getFacebook()->getApiClient()->setClientId(
            $rpcClientId
        );
     ...
     */
    public function getPageTypeFromRequest()
    {
        $rpcData = $this
                ->getServer()
                ->getRequest()
        ...
        $pageContext = Lib_Utils_Array::getProperty($rpcData, "page");
        $pageType = Lib_Utils_Array::getProperty($pageContext, "type");
        return $pageType;
    }
     ...
     * @return string|mixed|null
     */
    public function getPageUrlFromRequest()
    {
        $rpcData = $this
                ->getServer()
                ->getRequest()
        ...
        $pageUrl = Lib_Utils_Array::getProperty($pageContext, "url");
        return $pageUrl;
    }
