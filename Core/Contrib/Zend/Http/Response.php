...
use Zend\Stdlib\Message, Zend\Stdlib\ResponseDescription;
...
class Response extends Message implements ResponseDescription
{
     ...
     */
    protected $recommendedReasonPhrases = array(// INFORMATIONAL CODES
    100 => 'Continue', 
    101 => 'Switching Protocols', // SUCCESS CODES
    200 => 'OK', 201 => 'Created', 
...
307 => 'Temporary Redirect', // CLIENT ERROR
    ...
    public static function fromString ($string)
    {
        ...
        $matches = null;
        if (! preg_match(
        '/^HTTP\/(?P<version>1\.[01]) (?P<status>\d{3}) (?P<reason>.*)$/', 
        $firstLine, $matches)) {
        ...
        if (! defined(
        get_called_class() . '::STATUS_CODE_' . $matches['status'])) {
            throw new Exception\InvalidArgumentException(
            'Unknown status code found in provided string');
        }
    ...
    {
        return 'HTTP/' . $this->getVersion() . ' ' . $this->getStatusCode() . ' ' .
