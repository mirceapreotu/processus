...
class InstanceManager /* implements InstanceCollection */
{
     ...
     * @var array
     */
    protected $sharedInstancesWithParams = array('hashShort' => array(), 
    'hashLong' => array());
    ...
    public function addSharedInstance ($instance, $classOrAlias)
    {
        if (! is_object($instance)) {
            ...
            'This method requires an object to be shared');
        }
    ...
    array $params, $returnFashHashLookupKey = false)
    {
        ksort($params);
    ...
    public function setConfiguration ($aliasOrClass, array $configuration, 
    $append = false)
    {
        $key = ($this->hasAlias($aliasOrClass)) ? 'alias:' .
         $this->getBaseAlias($aliasOrClass) : $aliasOrClass;
    ...
    {
        return $this->setConfiguration($aliasOrClass, 
    ...
    {
        return $this->setConfiguration($aliasOrClass, 
        array('injections' => $injections), true);
    }

    public function hasTypePreferences ($interfaceOrAbstract)
    {
        $key = ($this->hasAlias($interfaceOrAbstract)) ? 'alias:' .
         $interfaceOrAbstract : $interfaceOrAbstract;
        return (isset($this->typePreferences[$key]) &&
