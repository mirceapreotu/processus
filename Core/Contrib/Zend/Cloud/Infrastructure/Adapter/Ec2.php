...
use Zend\Service\Amazon\Ec2\Instance as Ec2Instance, Zend\Service\Amazon\Ec2\Image as Ec2Image, Zend\Service\Amazon\Ec2\AvailabilityZones as Ec2Zone, Zend\Service\Amazon\Ec2\CloudWatch as Ec2Monitor, Zend\Service\Amazon\Ec2\Exception as Ec2Exception, Zend\Cloud\Infrastructure\Instance, Zend\Cloud\Infrastructure\InstanceList, Zend\Cloud\Infrastructure\Image, Zend\Cloud\Infrastructure\ImageList;
...
class Ec2 extends AbstractAdapter
{
     ...
     * @var array 
     */
    protected $mapStatus = array('running' => Instance::STATUS_RUNNING, 
    ...
    public function __construct ($options = array())
    {
        ...
        try {
            $this->ec2 = new Ec2Instance($options[self::AWS_ACCESS_KEY], 
            $options[self::AWS_SECRET_KEY], $this->region);
        ...
        if (isset($options[self::HTTP_ADAPTER])) {
            $this->ec2->getHttpClient()->setAdapter(
    ...
    public function createInstance ($name, $options)
    {
        ...
        $this->convertAttributes($this->adapterResult['instances'][0]));
    }
    ...
    {
        throw new Exception\RuntimeException(
        'The stopInstance method is not implemented in the adapter');
    }
    ...
    {
        throw new Exception\RuntimeException(
        'The startInstance method is not implemented in the adapter');
    }
    ...
    public function imagesInstance ()
    {
        if (! isset($this->ec2Image)) {
            $this->ec2Image = new Ec2Image($this->accessKey, $this->accessSecret, 
            $this->region);
        }
        ...
        foreach ($this->adapterResult as $result) {
            ...
            $images[] = array(Image::IMAGE_ID => $result['imageId'], 
            Image::IMAGE_NAME => '', 
    ...
    public function zonesInstance ()
    {
        if (! isset($this->ec2Zone)) {
            $this->ec2Zone = new Ec2Zone($this->accessKey, $this->accessSecret, 
            $this->region);
        }
    ...
    public function monitorInstance ($id, $metric, $options = null)
    {
        ...
        if (! in_array($metric, $this->validMetrics)) {
            throw new Exception\InvalidArgumentException(
            sprintf('The metric "%s" is not valid', $metric));
        }
        ...
        if (! empty($options) && (empty($options[Instance::MONITOR_START_TIME]) ||
         empty($options[Instance::MONITOR_END_TIME]))) {
            throw new Exception\InvalidArgumentException(
            sprintf('The options array must contain: "%s" and "%s"', 
            $options[Instance::MONITOR_START_TIME], 
            $options[Instance::MONITOR_END_TIME]));
        }
        
        if (! isset($this->ec2Monitor)) {
            $this->ec2Monitor = new Ec2Monitor($this->accessKey, 
            $this->accessSecret, $this->region);
        }
        
        $param = array('MeasureName' => $this->mapMetrics[$metric], 
