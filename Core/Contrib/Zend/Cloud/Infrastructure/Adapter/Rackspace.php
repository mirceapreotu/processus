...
use Zend\Service\Rackspace\Servers as RackspaceServers, Zend\Cloud\Infrastructure\Instance, Zend\Cloud\Infrastructure\InstanceList, Zend\Cloud\Infrastructure\Image, Zend\Cloud\Infrastructure\ImageList;
...
class Rackspace extends AbstractAdapter
{
    ...
    public function __construct ($options = array())
    {
        ...
        if (! isset($options[self::RACKSPACE_USER])) {
            throw new Exception\InvalidArgumentException(
            'Rackspace access user not specified!');
        }
        
        if (! isset($options[self::RACKSPACE_KEY])) {
            throw new Exception\InvalidArgumentException(
            'Rackspace access key not specified!');
        }
        ...
        try {
            $this->rackspace = new RackspaceServers($this->accessUser, 
            $this->accessKey, $this->region);
    ...
    public function createInstance ($name, $options)
    {
        if (empty($name)) {
            throw new Exception\InvalidArgumentException(
            'You must specify the name of the instance');
        }
        if (empty($options) || ! is_array($options)) {
            throw new Exception\InvalidArgumentException(
        ...
        $options['name'] = $name;
        $this->adapterResult = $this->rackspace->createServer($options, 
        $metadata, $files);
        if ($this->adapterResult === false) {
            return false;
        }
        return new Instance($this, 
        $this->convertAttributes($this->adapterResult->toArray()));
    }
    ...
    public function imagesInstance ()
    {
        ...
        foreach ($images as $image) {
            if (strtolower($image['status']) === 'active') {
                ...
                $result[] = array(Image::IMAGE_ID => $image['id'], 
    ...
    public function monitorInstance ($id, $metric, $options = null)
    {
        if (! function_exists("ssh2_connect")) {
            throw new Exception\RuntimeException(
            'Monitor requires the PHP "SSH" extension (ext/ssh2)');
        }
        if (empty($id)) {
            throw new Exception\InvalidArgumentException(
            'You must specify the id of the instance to monitor');
        }
        if (empty($metric)) {
            throw new Exception\InvalidArgumentException(
            'You must specify the metric to monitor');
        }
        ...
        switch ($metric) {
            case Instance::MONITOR_CPU:
                $cmd = 'top -b -n ' . self::MONITOR_CPU_SAMPLES .
                 ' | grep \'Cpu\'';
                break;
        ...
        if (empty($cmd)) {
            throw new Exception\InvalidArgumentException(
            'The metric specified is not supported by the adapter');
        }
        
        $params = array(Instance::SSH_USERNAME => $options['username'], 
        ...
        foreach ($outputs as $output) {
            if (! empty($output)) {
                ...
                $monitor['series'][] = array('timestamp' => $exec_time, 
