 ...
 */
class Pheanstalk_Command_PeekCommand extends Pheanstalk_Command_AbstractCommand implements 
Pheanstalk_ResponseParser
{
    ...
    private $_subcommands = array(self::TYPE_READY, self::TYPE_DELAYED, 
    self::TYPE_BURIED);
    ...
    public function __construct ($peekSubject)
    {
        if (is_int($peekSubject) || ctype_digit($peekSubject)) {
            ...
            throw new Pheanstalk_Exception_CommandException(
            sprintf('Invalid peek subject: %s', $peekSubject));
        }
    }
    ...
    {
        return isset($this->_jobId) ? sprintf('peek %d', $this->_jobId) : sprintf(
        'peek-%s', $this->_subcommand);
    }
    ...
    public function parseResponse ($responseLine, $responseData)
    {
        if ($responseLine == Pheanstalk_Response::RESPONSE_NOT_FOUND) {
            if (isset($this->_jobId)) {
                $message = sprintf('%s: Job %d does not exist.', $responseLine, 
                $this->_jobId);
            } else {
                $message = sprintf("%s: There are no jobs in the '%s' status", 
                $responseLine, $this->_subcommand);
            }
            
            throw new Pheanstalk_Exception_ServerException($message);
        } elseif (preg_match('#^FOUND (\d+) \d+$#', $responseLine, $matches)) {
            return $this->_createResponse(Pheanstalk_Response::RESPONSE_FOUND, 
            array('id' => (int) $matches[1], 'jobdata' => $responseData));
        }
    }
}
