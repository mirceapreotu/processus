 ...
 */
class Pheanstalk_Command_PutCommand extends Pheanstalk_Command_AbstractCommand implements 
Pheanstalk_ResponseParser
{
    ...
    {
        return sprintf('put %d %d %d %d', $this->_priority, $this->_delay, 
        $this->_ttr, $this->getDataLength());
    }
    ...
    public function parseResponse ($responseLine, $responseData)
    {
        if (preg_match('#^INSERTED (\d+)$#', $responseLine, $matches)) {
            ...
            throw new Pheanstalk_Exception(
            sprintf(
            '%s: server ran out of memory trying to grow the priority queue data structure.', 
            $responseLine));
        } elseif (preg_match('#^JOB_TOO_BIG$#', $responseLine)) {
            throw new Pheanstalk_Exception(
            sprintf('%s: job data exceeds server-enforced limit', $responseLine));
        } elseif (preg_match('#^EXPECTED_CRLF#', $responseLine)) {
            throw new Pheanstalk_Exception(
            sprintf('%s: CRLF expected', $responseLine));
        } elseif (preg_match()) {
            throw new Pheanstalk_Exception(
            sprintf('Unhandled response: %s', $responseLine));
        }
    }
}
