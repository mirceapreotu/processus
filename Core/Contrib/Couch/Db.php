...
class Couch_Db
{
    ...
    {
        if (array_key_exists("curl", $this->options) &&
    ...
    public static function parseRawResponse ($raw_data, $json_as_array = FALSE)
    {
        ...
        if (strlen($body)) {
            $response['body'] = preg_match(
            '@Content-Type:\s+application/json@i', $headers) ? json_decode($body, 
            $json_as_array) : $body;
        }
        return $response;
    }
    ...
    {
        if ($this->curl) {
            return $this->_curl_query($method, $url, $parameters, $data);
        } else {
            return $this->_socket_query($method, $url, $parameters, $data);
        }
    }
     ...
     * @return string|false server response on success, false on error
     */
    public function continuousQuery ($callable, $method, $url, 
    $parameters = array(), $data = null)
    {
        ...
        if (! is_callable($callable)) {
            ...
            "callable argument have to success to is_callable PHP function");
        }
        ...
        while ($this->socket && ! feof($this->socket)) {
            ...
            if (false ===
             ($num_changed_streams = stream_select($read, $e, $e2, 1))) {
                $this->socket = null;
    ...
    protected function _socket_startRequestHeaders ($method, $url)
    {
        ...
        if ($this->dsn_part('user') && $this->dsn_part('pass')) {
            ...
            $this->dsn_part('user') . ':' . $this->dsn_part('pass')) . "\r\n";
        }
    ...
    protected function _socket_storeFile ($url, $file, $content_type)
    {
        ...
        if (! strlen($file) or ! is_file($file) or ! is_readable($file)) {
            throw new InvalidArgumentException(
            "Attachment file does not exist or is not readable");
        }
        if (! strlen($content_type)) {
            throw new InvalidArgumentException(
            "Attachment Content Type can't be empty");
        }
    ...
    protected function _connect ()
    {
        ...
        $this->socket = @fsockopen($ssl . $this->dsn_part('host'), 
        $this->dsn_part('port'), $err_num, $err_string);
        if (! $this->socket) {
            throw new Exception(
            'Could not open connection to ' . $this->dsn_part('host') . ':' .
             $this->dsn_part('port') . ': ' . $err_string . ' (' . $err_num . ')');
            return FALSE;
        }
        return TRUE;
    }
    ...
    public function _curl_storeFile ($url, $file, $content_type)
    {
        ...
        $http_headers = array(
        'Accept: application/json,text/html,text/plain,*/*', 
        'Content-Type: ' . $content_type, 'Expect: ');
