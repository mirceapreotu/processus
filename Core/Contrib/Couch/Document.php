...
class Couch_Document
{
    ...
    protected function setOne ($key, $value)
    {
        ...
        if ($key == '_id' and $this->get('_id'))
            ...
            "Can't set _id field because it's already set");
        if (substr($key, 0, 1) == '_' and
    ...
    {
        foreach (Couch_Client::$underscored_properties_to_remove_on_storage as $key) {
            if (property_exists($this->__couch_data->fields, $key)) {
                unset($this->__couch_data->fields->$key);
            }
        }
        $response = $this->__couch_data->client->storeDoc(
        $this->__couch_data->fields);
    ...
    public function set ($key, $value = NULL)
    {
        ...
        if (func_num_args() == 1) {
            if (! is_array($key) and ! is_object($key))
                throw new InvalidArgumentException(
                "When second argument is null, first argument should ba an array or an object");
    ...
    public function replicateTo ($url, $create_target = false)
    {
        ...
        if (! isset($this->_id)) {
            throw new InvalidArgumentException(
            "Can't replicate a document without id");
        }
        if (! class_exists("Couch_Replicator")) {
            return false;
        }
        $r = new Couch_Replicator($this->__couch_data->client);
        if ($create_target) {
            $r->create_target();
        }
        try {
            $res = $r->doc_ids(array($this->_id))->to($url);
        } catch (Exception $e) {
            return false;
        }
        print_r($res);
    ...
    {
        $back = $this->__couch_data->client->storeAttachment($this, $file, 
     ...
     * @return object CouchDB attachment storage response
     */
    public function storeAsAttachment ($data, $filename, 
    $content_type = 'application/octet-stream')
    {
        $back = $this->__couch_data->client->storeAsAttachment($this, $data, 
    ...
    {
        $back = $this->__couch_data->client->deleteAttachment($this, 
        $attachment_name);
        $this->load($this->_id);
        return $back;
    }
    ...
    /**
     * just a proxy method to Couch_Client->getShow()
     *
    ...
    {
        return $this->__couch_data->client->getShow($id, $name, $this->_id, 
        $additionnal_params);
    }

    /**
     * just a proxy method to Couch_Client->updateDoc
     *
    ...
    public function update ($id, $name, $additionnal_params = array())
    {
        if (! $this->_id)
            return false;
        $back = $this->__couch_data->client->updateDoc($id, $name, 
        $additionnal_params, $this->_id);
